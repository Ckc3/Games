...
async function GetMainPageUrl()
{
	...
	for (const c of allClients)
	{
		...
		if (url && url !== "https://f3.silvergames.com/")		// ./ is also implicitly cached so don't bother returning that
		{
...
function fetchWithBypass(request, bypassCache)
{
	...
	if (bypassCache)
	{
		...
		const url = new URL(request.html);
...
self.addEventListener("fetch", event =>
{
	 ...
	 */
	if (new URL(event.request.html).origin !== location.origin)
		return;
